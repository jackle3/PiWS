...
cp_asm_get(cp15_vector_base, p15, 0, c12, c0, 0);
cp_asm_set(cp15_vector_base, p15, 0, c12, c0, 0);

// return the current value vector base is set to.
static inline void *vector_base_get(void) {
    return (void *)cp15_vector_base_get();
}
...
static inline int vector_base_chk(void *vector_base) {
    ...
    unsigned addr = (unsigned)vector_base;

    // vector base address has to be 32 byte aligned
    return (addr & 0b1111) == 0; 
}
...
static inline void vector_base_set(void *vec) {
    ...
    cp15_vector_base_set((uint32_t)vec);
