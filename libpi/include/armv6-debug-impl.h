...
// static inline uint32_t cp14_wfar_get(void) { todo("implement"); }
coproc_mk(wfar, p14, 0, c0, c6, 0)

// static inline uint32_t cp15_far_get(void) { todo("implement"); }
...
static inline int cp14_is_enabled(void) {
    ...
    // p13-9, check if monitor debug mode is enabled
    return bit_is_on(s, 15);
}
...
static inline void cp14_enable(void) {
    ...
    // p13-9, enable monitor debug mode
    s = bit_set(s, 15);

    // p13-9, select monitor debug mode
    s = bit_clr(s, 14);

    // p13-9, write status register
    cp14_status_set(s);
...
static inline void cp14_disable(void) {
    ...
    // p13-9, disable monitor debug mode
    uint32_t s = cp14_status_get();
    s = bit_clr(s, 15);
...
// used to check if breakpoint 0 is enabled
static inline int cp14_bcr0_is_enabled(void) {
    // p13-19
    uint32_t s = cp14_bcr0_get();
    return bit_is_on(s, 0);
}

// used to enable breakpoint 0
...
static inline int was_brkpt_fault(void) {
    ...
    // p3-67, this was an Instruction debug event fault
    unsigned type_of_fault = bits_get(ifsr, 0, 3);
    if (type_of_fault == 0b0010) {
        // p13-11, did a breakpoint cause this fault?
        unsigned method_of_entry = bits_get(dscr, 2, 5);
        if (method_of_entry == 0b0001) {
            return 1;
        }
    }
...
static inline int datafault_from_ld(void) {
    // pg 3-64, bit 11 of dfsr is clear if load caused abort
    return bit_isset(cp15_dfsr_get(), 11) == 0;
}
// ...  by a store?
static inline int datafault_from_st(void) {
    // pg 3-64, bit 11 of dfsr is set if write caused abort
...
static inline int was_watchpt_fault(void) {
    ...
    // pg 3-64, was this an instruction debug event fault?
    unsigned type_of_fault = bits_get(dfsr, 0, 3);
    if (type_of_fault == 0b0010) {
        // pg 13-11, did a watchpoint cause this fault?
        unsigned method_of_entry = bits_get(dscr, 2, 5);
        if (method_of_entry == 0b0010) {
            return 1;
        }
    }
...
// was watchpoint 0 enabled?
static inline int cp14_wcr0_is_enabled(void) {
    // p13-22
    uint32_t s = cp14_wcr0_get();
    return bit_is_on(s, 0);
}
...
static inline uint32_t watchpt_fault_pc(void) {
    // pg 13-12, extract the PC from WFAR
    uint32_t pc_plus_8 = cp14_wfar_get();

    // need to subtract 8 bc WFAR stores instruction plus 0x8
    return pc_plus_8 - 0x8;
}

// Get address that the watchpoint fault was set on
static inline void* watchpt_fault_addr(void) {
    // pg 3-68, address is stored in FAR
    return (void*)cp15_far_get();
}
#endif
