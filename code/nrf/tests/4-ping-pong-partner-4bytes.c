...
enum { ntrial = 5, timeout_usec = 1000, nbytes = 4 };
...
ping_pong_ack(nrf_t *s, nrf_t *c, int verbose_p) {
    ...
    for(unsigned i = 0; i < ntrial; i++) {
        ...
        // I act as the client, Alex acts as the server

        // Alex sends first so I need to wait for a second
        delay_ms(1000);

        // Client RECEIVING: Waiting for a packet from server
        // If we don't receive a packet (timeout), increment timeout counter
        if(!net_get32(c, &got))
            ntimeout++;
        ...
        // Client SENDING: After receiving (or timing out), client sends next value
