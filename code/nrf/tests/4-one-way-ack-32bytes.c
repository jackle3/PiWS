// one way test of 32-byte ack'd packets.  right now if it gets duplicate
...
one_way_ack(nrf_t *server, nrf_t *client, int verbose_p) {
    ...
    for(unsigned i = 0; i < ntrial; i++) {
        ...
        uint32_t x;
        if(nrf_read_exact_timeout(client, recv, sizeof recv, timeout_usec) == 32) {
